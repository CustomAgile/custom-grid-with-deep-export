...
module.exports = function(grunt) {
    ...
    if (grunt.file.exists(auth_file_name)) {
        let auth = grunt.file.readJSON(auth_file_name);
        config.auth = auth;
    } else {
        grunt.log.writeln('');
    ...
    grunt.registerTask('putUglyInConfig', 'add results of uglification to the config', () => {
        config.ugly_contents = grunt.file.read('deploy/app.min.js');
    });

    grunt.registerTask('setPostBuildInfo', 'Make a sloppy checksum', (deploy_file_name) => {
        var fs = require('fs'),
    ...
    grunt.registerTask('install', 'Deploy the app to a rally instance', function(deploy_file_name) {
        ...
        let installApp = function(page_oid, panel_oid) {
            let html = grunt.file.read(deploy_file_name);

            let uri = `${config.auth.server  }/slm/dashboard/changepanelsettings.sp`;
        ...
        let makeApp = function(key, page_oid) {
            let uri = `${config.auth.server  }/slm/dashboard/addpanel.sp`;
        ...
        let makePage = function(key) {
            let uri = `${config.auth.server  }/slm/wt/edit/create.sp`;
            let parameters = {
                cpoid: 729766,
                key
            };
        ...
        let uri = `${config.auth.server  }/slm/webservice/v2.0/security/authorize`;
